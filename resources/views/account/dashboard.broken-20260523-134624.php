...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-5">
                ...
                <p class="text-2xl font-extrabold text-gray-900 mt-2">{{ $unfinishedOrdersCount ?? 0 }}</p>
                    ...
                    @forelse($sales->take(6) as $sale)
                        <div class="p-4 flex gap-4">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="font-bold text-gray-900 truncate">{{ $sale->listing->title ?? '{{ $sale->listing->title ?? 'Annonce indisponible' }}' }}</p>
                                ...
                                @if($sale->tracking_number)
                                    <p class="text-xs text-gray-500 mt-1">
                                        {{ $sale->carrier }} · {{ $sale->tracking_number }}
                                    </p>

                                    @if($sale->tracking_url)
                                        <a href="{{ $sale->tracking_url }}" target="_blank" class="text-xs text-teal-700 font-bold">
                                            Suivre le colis →
                                        </a>
                                    @endif
                                @endif
                                ...
                                @if($sale->status === 'completed')
                                    ...
                                    @if($sale->wallet_status === 'paid')
                                        ...
                                        <div class="mt-2">
                                            ...
                                            <p class="text-[11px] text-green-600 mt-1">
                                                Le paiement a été envoyé sur votre compte bancaire.
                                            </p>
                                        </div>
                                    ...
                                    @elseif(!$user->stripe_account_id || !$user->stripe_payouts_enabled)
                                        ...
                                        <div class="mt-2">
                                            ...
                                            <p class="text-[11px] text-gray-500 mt-1">
                                                Ajoutez votre IBAN pour recevoir vos ventes.
                                            </p>
                                        </div>
                                    ...
                                    @else
                                        ...
                                        <div class="mt-2">
                                            ...
                                            <p class="text-[11px] text-gray-500 mt-1">
                                                Le paiement sera envoyé automatiquement.
                                            </p>
                                        </div>
                            ...
                            <div class="flex flex-col items-end gap-2">
                                ...
                                <span class="text-xs font-bold px-3 py-1 rounded-full
                                    ...
                                    {{ ['paid'=>'Payé','shipped'=>'Expédié','received'=>'Reçu','completed'=>'Terminée','pending'=>'Payée'][$sale->shipping_status ?? $sale->status] ?? ($sale->shipping_status ?? $sale->status) }}
                                </span>

                                @if($sale->payment_method === 'cb' && ($sale->shipping_status ?? null) !== 'shipped' && $sale->status === 'paid')
                                    <details class="w-56">
                                        <summary class="cursor-pointer px-3 py-2 rounded-xl bg-teal-700 text-white text-xs font-bold text-center">
                                            Expédier
                                        </summary>

                                        <form method="POST" action="{{ route('transactions.shipped', $sale) }}" class="mt-2 bg-gray-50 border border-gray-100 rounded-2xl p-3 space-y-2">
                                            @csrf
                                            @method('PATCH')

                                            <select name="carrier" class="w-full text-xs rounded-xl border-gray-200">
                                            ...
                                            <input name="tracking_number" type="text" placeholder="N° suivi" class="w-full text-xs rounded-xl border-gray-200">

                                            <button class="w-full px-3 py-2 rounded-xl bg-teal-700 text-white text-xs font-bold">
                                ...
                                @if($sale->payment_method !== 'cb' && $sale->status === 'paid')
                                    <p class="text-[11px] text-gray-500 max-w-40 text-right">
                                        Livraison Colissimo non disponible pour ce type de transaction.
                                    </p>
                                @endif
                ...
                <div class="divide-y divide-gray-100">
                    @forelse($purchases->take(6) as $purchase)
                        <div class="p-4 flex gap-4">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="font-bold text-gray-900 truncate">{{ $purchase->listing->title ?? '{{ $sale->listing->title ?? 'Annonce indisponible' }}' }}</p>
                                ...
                                @if($purchase->tracking_number)
                                    <p class="text-xs text-gray-500 mt-1">
                                        {{ $purchase->carrier }} · {{ $purchase->tracking_number }}
                                    </p>

                                    @if($purchase->tracking_url)
                                        <a href="{{ $purchase->tracking_url }}" target="_blank" class="text-xs text-teal-700 font-bold">
                                            Suivre le colis →
                                        </a>
                                    @endif
                                @endif
                            ...
                            <div class="flex flex-col items-end gap-2">
                                ...
                                <span class="text-xs font-bold px-3 py-1 rounded-full
                                    ...
                                    @else bg-gray-100 text-gray-700 @endif">
                                    {{ ['paid'=>'Payé','shipped'=>'Expédié','received'=>'Reçu','completed'=>'Terminée','pending'=>'Payée'][$purchase->shipping_status ?? $purchase->status] ?? ($purchase->shipping_status ?? $purchase->status) }}
                                </span>
...
<div class="mt-6 bg-white rounded-3xl border border-gray-100 overflow-hidden">
    ...
    @forelse(($pendingCheckouts ?? collect())->take(5) as $sale)
        <div class="p-4 border-b border-gray-100 flex items-center justify-between gap-3">
            <div>
                <p class="font-bold text-gray-900">{{ $sale->listing->title ?? '{{ $sale->listing->title ?? 'Annonce indisponible' }}' }}</p>
