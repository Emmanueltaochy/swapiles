    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            ...
            <p class="text-gray-500 mt-2">
                Ajoutez votre article en quelques minutes.
            </p>
        </div>
        ...
        <form method="POST" action="{{ route('account.listings.store') }}" enctype="multipart/form-data" class="bg-white rounded-3xl shadow-sm border border-gray-100 p-5 sm:p-7 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="listing_type" required class="w-full rounded-2xl bg-gray-100 border-0 px-4 py-3 focus:ring-2 focus:ring-teal-600">
                        <option value="achat" @selected(old('listing_type') === 'achat')>🔒 Vente avec paiement protégé</option>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="category_level1" required class="w-full rounded-2xl bg-gray-100 border-0 px-4 py-3 focus:ring-2 focus:ring-teal-600">
                        ...
                        <option value="Femme" @selected(old('category_level1') === 'Femme')>Femme</option>
                        <option value="Homme" @selected(old('category_level1') === 'Homme')>Homme</option>
                        <option value="Enfant" @selected(old('category_level1') === 'Enfant')>Enfant</option>
                        <option value="Accessoires" @selected(old('category_level1') === 'Accessoires')>Accessoires</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="territoire" required class="w-full rounded-2xl bg-gray-100 border-0 px-4 py-3 focus:ring-2 focus:ring-teal-600">
                        <option value="La Réunion" @selected(old('territoire', 'La Réunion') === 'La Réunion')>🇷🇪 La Réunion</option>
                        <option value="Martinique" @selected(old('territoire') === 'Martinique')>🇲🇶 Martinique</option>
                        <option value="Guadeloupe" @selected(old('territoire') === 'Guadeloupe')>🇬🇵 Guadeloupe</option>
                        <option value="Guyane" @selected(old('territoire') === 'Guyane')>🇬🇫 Guyane</option>
                        <option value="Mayotte" @selected(old('territoire') === 'Mayotte')>🇾🇹 Mayotte</option>
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <input type="number" name="shipping_price" value="{{ old('shipping_price') }}" min="0" placeholder="Ex : 5" class="w-full rounded-2xl bg-gray-100 border-0 px-4 py-3 focus:ring-2 focus:ring-teal-600">
            </div>
