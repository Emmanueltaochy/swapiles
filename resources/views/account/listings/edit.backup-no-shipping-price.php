    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @if($listing->images->count())
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-5 sm:p-7 mb-6">
                ...
                <div class="grid grid-cols-3 sm:grid-cols-5 gap-3">
                    @foreach($listing->images as $image)
                        <div class="relative group">
                            ...
                            @if($image->order === 0)
                                <span class="absolute left-2 top-2 bg-teal-700 text-white text-[10px] font-bold px-2 py-1 rounded-full shadow">
                                    Principale
                                </span>
                            @else
                                <form method="POST"
                                      action="{{ route('account.listings.images.main', ['listing' => $listing, 'image' => $image]) }}"
                                      class="absolute left-2 top-2">
                                    @csrf
                                    @method('PATCH')

                                    <button class="bg-white/95 text-gray-800 text-[10px] font-bold px-2 py-1 rounded-full shadow">
                                        Mettre en principal
                                    </button>
                                </form>
                            @endif
                            ...
                            <form method="POST"
                                  action="{{ route('account.listings.images.destroy', ['listing' => $listing, 'image' => $image]) }}"
                                  onsubmit="return confirm('Supprimer cette photo ?');"
                                  class="absolute top-2 right-2">
                                @csrf
                                @method('DELETE')

                                <button class="w-8 h-8 rounded-full bg-red-600 text-white flex items-center justify-center shadow text-sm font-bold">
                                    ×
                                </button>
                            </form>
                        </div>
                ...
                <p class="text-xs text-gray-500 mt-3">Clique sur × pour supprimer une photo. Tu peux aussi ajouter de nouvelles photos plus bas.</p>
            </div>
        @endif
