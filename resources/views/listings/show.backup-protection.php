...
@php
    $protectionFee = max(1, round(($listing->price ?? 0) * 0.10, 2));
    $protectedTotal = ($listing->price ?? 0) + $protectionFee;
@endphp
...
<div class="mt-5 rounded-3xl bg-gray-50 border border-gray-100 p-4">
    ...
    @if($listing->allows_colissimo ?? false)
        <button type="button"
            ...
            <span class="block text-sm font-bold text-teal-700">
                Prix avec protection : {{ number_format($protectedTotal, 2, ',', ' ') }} € 🛡️
            </span>
            <span class="block text-xs text-gray-500">
                Livraison Colissimo calculée au paiement.
            </span>
        </button>
    @endif
    ...
    <div class="mt-3 flex flex-wrap gap-2">
        ...
        @if($listing->allows_hand_delivery ?? true)
                        ...
                        <div class="mt-5">
                            ...
                            @if($listing->listing_type === 'achat')
                                <p class="mt-1 text-sm text-teal-700">
                                    Protection acheteur ajoutée au paiement 🛡️
                                </p>
                            @endif
                        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const phrases = [
        ...
        '🔥 Plusieurs personnes ont regardé cette annonce aujourd’hui',
        '✨ Article unique : un seul disponible',
        '⏳ Les bonnes affaires partent vite sur Swap’Îles',
        '💚 Ajoutez-le à vos favoris pour ne pas le perdre',
        ''
    ];
