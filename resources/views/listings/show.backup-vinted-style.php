...
@endsection
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
        <a href="{{ route('profiles.show', $listing->user) }}" class="block bg-white border border-gray-100 rounded-3xl p-5 shadow-sm hover:shadow-md transition">
            <div class="flex items-center gap-4">
                <div class="w-14 h-14 rounded-full bg-teal-100 flex items-center justify-center overflow-hidden font-extrabold text-teal-800">
                    ...
                    @else
                        {{ $listing->user ? strtoupper(substr($listing->user->name, 0, 1)) : 'S' }}
                    @endif
                </div>
                ...
                <div>
                    ...
                    <p class="font-extrabold text-gray-900">
                        {{ $listing->user->name ?? 'Utilisateur Swap\'Îles' }}
                    </p>
                    <p class="text-xs text-gray-500">
                        Voir le profil et ses annonces →
                    </p>
                </div>
            </div>
        </a>
